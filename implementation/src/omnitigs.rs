...
pub(crate) fn perform_linear_reduction<
    Graph: DynamicBigraph + StaticEdgeCentricBigraph + Default + Clone,
>(
    graph: &Graph,
    force_sc: bool,
    ...
    <Graph as ImmutableGraphContainer>::NodeIndicesCopied: DoubleEndedIterator + ExactSizeIterator,
{
    ...
    for node in reduced.node_indices_copied().rev().skip(1).rev() {
...
pub(crate) fn split_walks_at_node<Graph: StaticGraph>(
...
fn compute_omnitigs_from_graph<
