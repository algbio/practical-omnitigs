    ...
    fn compute_maximal_microtigs(
        ...
        for macronode in macronodes {
            ...
            let first_edge = graph
                .in_neighbors(macronode.iter().next().unwrap())
                .next()
                .expect(
            ...
            let r_minus: BitVectorSubgraph<_> =
                compute_restricted_backward_reachability(graph, first_edge);

            // TODO
            drop(r_minus);

            let rmm1 = VecEdgeWalk::<Graph>::from(rmm1);
